...
namespace Anthurium.Web.Services {
  public class ItemService : IItemService {
    ...
    public ItemService(IHttpService httpService) { _httpService = httpService; }

    public async Task<ItemApiResponse> GetItemsAsync(string orderBy, int skip,
                                                     int top) {
      return await _httpService.Get<ItemApiResponse>(
          $"api/item?$count=true&$expand=Warehouse&$orderby={orderBy}&$skip={skip}&$top={top}");
    }

    public async Task<ItemApiResponse> GetItemsAsync(string orderBy, int skip) {
      return await _httpService.Get<ItemApiResponse>(
          $"api/item?$count=true&$expand=Warehouse&$orderby={orderBy}&$skip={skip}");
    }

    public async Task<ItemReadDto> GetItemByIdAsync(int id) {
      return await _httpService.Get<ItemReadDto>(
          $"api/item/{id}?$expand=Warehouse");
    }
