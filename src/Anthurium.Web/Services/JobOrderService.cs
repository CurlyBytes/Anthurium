...
namespace Anthurium.Web.Services
{
    public class JobOrderService : IJobOrderService
    {
        ...
        {
            return await _httpService.Get<JobOrderApiResponse>($"api/joborder?$count=true&$orderby={orderBy}&$skip={skip}&$top={top}");
        ...
        {
            return await _httpService.Get<JobOrderApiResponse>($"api/joborder?$count=true&$filter=ClientInformationId eq {clientInformationId}&$orderby={orderBy}&$skip={skip}&$top={top}");
