...
namespace Anthurium.Web.Services {
  public class WarehouseService : IWarehouseService {
    ...
    public async Task<WarehouseApiResponse>
    GetWarehousesAsync(string orderBy, int skip, int top) {
      var test = await _httpService.Get<WarehouseApiResponse>(
          $"api/warehouse?$count=true&$orderby={orderBy}&$skip={skip}&$top={top}");

      return test;
    }

    public async Task<WarehouseApiResponse> GetWarehousesAsync(string orderBy,
                                                               int skip) {
      var test = await _httpService.Get<WarehouseApiResponse>(
          $"api/warehouse?$count=true&$orderby={orderBy}&$skip={skip}");
      return test;
    }
    ...
    CreateWarehouseAsync(WarehouseCreateDto warehouse) {

      return await _httpService.Post<HttpResponseMessage>($"api/warehouse",
                                                          warehouse);
    }
