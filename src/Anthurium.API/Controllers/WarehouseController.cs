...
namespace Anthurium.API.Controllers {
  ...
  public class WarehouseController : ODataController {
    ...
    public WarehouseController(ISqlServerWarehouseRepository repository,
                               IMapper mapper) {
    ...
    [EnableQuery]
    [ODataRoute("({id})")]
    public ActionResult<WarehouseReadDto>
    WarehouseById([ FromODataUri ] int id) {
      if (!ModelState.IsValid) {
        return BadRequest(ModelState);
      }
    ...
    [ODataRoute]
    public ActionResult<WarehouseReadDto>
    CreateCommand([ FromBody ] WarehouseCreateDto commandCreate) {
      ...
      WarehouseReadDto WarehouseReadDto =
          _mapper.Map<WarehouseReadDto>(commandItem);

      return CreatedAtRoute(nameof(WarehouseById),
                            new {Id = WarehouseReadDto.WarehouseId},
                            WarehouseReadDto);
    }
    ...
    [ODataRoute("({id})")]
    public ActionResult
    UpdateWarehouse([ FromODataUri ] int id,
    ...
    public ActionResult PartialUpdate([ FromODataUri ] int id,
                                      Delta<WarehouseUpdateDto> patchDocument) {
      if (!ModelState.IsValid) {
        return BadRequest(ModelState);
      }
      var commandItem = _repository.WarehouseById(id);
      ...
      WarehouseUpdateDto commandToPatch =
          _mapper.Map<WarehouseUpdateDto>(commandItem);
