...
public class ItemController : ODataController
{
    ...
    [HttpPost]
    [ODataRoute]
    public ActionResult<ItemReadDto> CreateCommand([FromBody] ItemCreateDto commandCreate)
    {
        ...
        return CreatedAtRoute(nameof(ItemById), new { Id = ItemReadDto.ItemId }, ItemReadDto);
    }

    [HttpPut("{id}")]
    [ODataRoute("({id})")]
    public ActionResult UpdateItem([FromODataUri] int id, [FromBody] ItemUpdateDto ItemUpdateDto)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
    ...
    [ODataRoute("({id})")]
    [HttpPatch("{id}")]
    public ActionResult PartialUpdate([FromODataUri] int id, Delta<ItemUpdateDto> patchDocument)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
