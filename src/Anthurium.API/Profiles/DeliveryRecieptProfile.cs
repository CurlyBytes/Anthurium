...
namespace Anthurium.API.Profiles {
  public class DeliveryReceiptProfile : Profile {
    public DeliveryReceiptProfile() {
      ...
      CreateMap<DeliveryReceiptCreateDto, DeliveryReceipt>().ForMember(
          destination => destination.DeliveryReceiptId,
          source => source.Ignore());

      CreateMap<DeliveryReceiptUpdateDto, DeliveryReceipt>()
          .ForMember(destination => destination.DeliveryReceiptId,
                     source => source.Ignore())
          .ForMember(destination => destination.DateCreated,
                     source => source.Ignore())
